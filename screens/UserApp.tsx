  ...
  const RenderHome = () => (
    <>
      <div className="absolute top-0 left-0 right-0 z-20 p-4 flex justify-between items-start bg-gradient-to-b from-black/50 to-transparent pb-12 pointer-events-none">
        <button onClick={() => setShowMenu(true)} className="bg-white p-2 rounded-full shadow-md text-gray-700 pointer-events-auto active:scale-90 transition"><Menu size={24} /></button>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 bg-white rounded-t-3xl shadow-2xl p-5 pb-8">
        ...
        {paymentFeedback && (
          <div className={`mb-4 p-4 rounded-xl flex items-center gap-3 animate-fade-in ${
            paymentFeedback.type === 'success' ? 'bg-green-50 text-green-800 border border-green-200' : 'bg-red-50 text-red-800 border border-red-200'
          }`}>
