...
export const calculateRoute = async (
  ...
  // 1. Tentar Google Directions API
  if (hasValidKey() && typeof window !== 'undefined' && window.google && window.google.maps) {
      try {
          ...
          const result: any = await new Promise((resolve, reject) => {
              directionsService.route({
                  ...
                  drivingOptions: {
                    ...
                    trafficModel: 'bestguess' // CORREÇÃO: 'best_guess' (incorreto) -> 'bestguess' (correto)
                  }
          ...
          console.warn("API de Rotas falhou (pode não estar habilitada). Usando cálculo simulado.", error);
      }
  }
